<?php
    ...
    /* 
    @版本日期: 2012年8月29日19:18:48
    @著作权所有: 1024 intelligence ( http://www.1024i.com ) 
    获得使用本类库的许可, 您必须保留著作权声明信息. 
    报告漏洞，意见或建议, 请联系 yuan ji(fenglangyj@139.com) 
	
	调用方法=======================================
	
include_once('Imagick.class.php');
$image= new lib_image_imagick();  
$image->open('data/test.gif');//调用图片
$image->resize_to(100,100,'scale');//改变大小
$image->add_text('shuyeer.com', 10, 20);//添加文字水印
//$image->add_watermark('test.jpg', 10, 50);//添加图片水印
$image->save_to('example_small.gif');//保存到指定路径
$im->output();//输出
    ...
    {  
        ...
        */  
        ...
        public function output($header = true){
            if($header) header('Content-type: image/'.$this->type);
