...
private function global_init(){ 
	...
	global $_cfg;$controller=$this->global_controller($FilePath);
	$controller->_cfg=&$_cfg; 
	if(method_exists($controller,ROUTE_A))
	{ 
		call_user_func(array($controller,ROUTE_A)); 
	}
	else{ _error('Action does not exist.','...'); exit(); } } 
    ...
    private function global_start(){ 
		if(!System::load_sys_config('system','web_off')){ $admin_dir = System::load_sys_config('system','admindir'); if($admin_dir !== ROUTE_M){ echo htmlspecialchars_decode(System::load_sys_config('system','web_off_text')); exit; } } 
	} 
	private function global_end(){ 
		if(defined("G_BANBEN_ERROR")){ 
			$content = ob_get_contents();ob_end_clean(); preg_match_all("/<title>(.*)<\/title>/",$content,$rusult,PREG_PATTERN_ORDER); 
			if(!empty($rusult[1])){ 
